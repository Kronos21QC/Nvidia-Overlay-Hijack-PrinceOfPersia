...
VOID c_overlay::dest_device() {
	dest_render_target();
	// Call safe_release as global helper
...
VOID c_overlay::init_imgui() {
	...
	font->im_font = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Tahoma.ttf", 22.0f);
	font->font_size = 22.f;
	...
	menu_ticks(NULL)
{
    ...
    // avoiding window flags: The following code sets the overlay window to be topmost and layered,
    // and extends its frame into the client area to avoid standard window flags that might interfere
    // with overlay rendering. This technique is commonly used to ensure the overlay remains visible
    // above other windows, including games, without triggering unwanted focus or z-order changes.
	...
	procID = Memory::Ex::GetProcessIdByName("TheLostCrown_plus.exe");
	if (!procID) {
#ifdef _DEBUG
		std::cerr << "[DEBUG] Failed to find process ID for TheLostCrown_plus.exe" << std::endl;
		system("pause");
#endif
		return;
	}
	...
	if (!hProcess) {
#ifdef _DEBUG
		std::cerr << "[DEBUG] Failed to open process handle." << std::endl;
		system("pause");
#endif
		return;
	}

	uintptr_t clientBase = Memory::Ex::GetModuleAddress(procID, "TheLostCrown_plus.exe");
	if (!clientBase) {
#ifdef _DEBUG
		std::cerr << "[DEBUG] Couldn't find module (TheLostCrown_plus.exe)." << std::endl;
		system("pause");
#endif
		return;
	}
	else {
		// This line explicitly shows the hex pointer value
#ifdef _DEBUG
		std::cout << "[DEBUG] Module pointer (TheLostCrown_plus.exe): "
			<< "0x" << std::uppercase << std::hex << clientBase << std::endl;
#endif
	}
	...
	std::thread(&c_overlay::inithacks, this).detach(); // init pointers
}
...
BOOL c_overlay::msg_loop() {
	if (window_handle) {
		...
		render(window_width, window_height); // Menu Render
...
void c_overlay::anim_handler() {
    ...
    bool last_menu_state = show_menu;
    ...
    for (;!exit;) {
        ...
        if (hProcess && GetExitCodeProcess(hProcess, &exitCode)) {
            ...
            // If handle is invalid, also exit
            exit = true;
        }
        // ---------------------------------------------
        ...
        // Show/hide cursor and toggle mouse passthrough when menu state changes
        if (show_menu != last_menu_state) {
            ShowCursor(show_menu ? TRUE : FALSE);
            set_mouse_passthrough(!show_menu); // Only allow mouse input when menu is open
...
void c_overlay::set_mouse_passthrough(bool passthrough) {
	LONG exStyle = GetWindowLongA(window_handle, GWL_EXSTYLE);
	if (passthrough)
		exStyle |= WS_EX_TRANSPARENT;
	else
		exStyle &= ~WS_EX_TRANSPARENT;
	SetWindowLongA(window_handle, GWL_EXSTYLE, exStyle);
}
...
void c_overlay::Menu() {
	...
	static bool l_Godmode = false;
	static bool l_InfPotion = false;
	...
	ImGui::SetWindowSize(ImVec2(560, 400));
	ImGui::Begin("KronosQC_ PrinceOfPercia Overlay", 0);
	//ImGui::Checkbox("Enable Crosshair", &l_crosshair);
	ImGui::Checkbox("Enable Show FPS", &l_show_fps);
	ImGui::Checkbox("Enable God Mode", &l_Godmode);
	ImGui::Checkbox("Enable Infinite Potion", &l_InfPotion);
	ImGui::End();
